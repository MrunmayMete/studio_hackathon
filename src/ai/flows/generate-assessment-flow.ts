...
const GenerateAssessmentInputSchema = z.object({
  ...
  competencyLevel: z.string().describe('The self-assessed competency level of the user (e.g., Beginner, Intermediate).'),
});
...
const generateAssessmentPrompt = ai.definePrompt({
  ...
  prompt: `You are a vocational education expert. Generate a 5-question multiple-choice diagnostic assessment for a learner with the following profile. The questions should gauge their foundational knowledge. For each question, provide a specific, narrow topic. The question difficulty should be tailored to the user's competency level.
