...
const GenerateAssessmentOutputSchema = z.object({
  questions: z.array(QuestionSchema).describe('An array of 5 generated assessment questions.'),
});
...
const generateAssessmentPrompt = ai.definePrompt({
  ...
  prompt: `You are a vocational education expert. Generate a 5-question multiple-choice diagnostic assessment for a learner with the following profile. The questions should gauge their foundational knowledge.

  Domain of Interest: {{{domain}}}
  Learning Goals: {{#each goals}}{{{this}}}{{/each}}

  Generate exactly 5 multiple-choice questions. Each question must have 4 options and one correct answer.`,
});
